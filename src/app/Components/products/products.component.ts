import { Component, EventEmitter, Input, Output } from '@angular/core';
import { Route, Router } from '@angular/router';
import { ICategory } from 'src/app/Models/icategory';
import { DiscountOffers, IProduct } from 'src/app/Models/iproduct';
import { Store } from 'src/app/Models/store';
import { ProductsService } from 'src/app/Services/products.service';

@Component({
  selector: 'app-products',
  templateUrl: './products.component.html',
  styleUrls: ['./products.component.scss'],
})
export class ProductsComponent {
  // product: Store = new Store(
  //   'Mobile Note S22 Ultra',
  //   ['port said', 'cairo'],
  //   'https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcR0bgooGl7xcP8sdg07Eb8P4i2YPipKx52miBm3jqGni3H8Ib6Haq_mYDNbjXQ7Cm8oCOyqgzTeAcUdkFilihBvSUol3AplYc42SgcAovzEWHlfQ0RdatN-CQ&usqp=CAc'
  // );

  // Discount = DiscountOffers;
  ClientName: string = 'Ahmed';
  // productsList: IProduct[];
  filteredList: IProduct[] = [];
  // categoryList: ICategory[];
  showQuantity: boolean = true;
  show: boolean = true;
  credit: string = '';

  constructor(private prodService: ProductsService, private router: Router) {
    // this.productsList = [
    //   {
    //     ID: 1,
    //     Name: 'SAMSUNG Note S22 Ultra',
    //     Quantity: 3,
    //     Price: 22500,
    //     Img: 'https://encrypted-tbn0.gstatic.com/shopping?q=tbn:ANd9GcR0bgooGl7xcP8sdg07Eb8P4i2YPipKx52miBm3jqGni3H8Ib6Haq_mYDNbjXQ7Cm8oCOyqgzTeAcUdkFilihBvSUol3AplYc42SgcAovzEWHlfQ0RdatN-CQ&usqp=CAc',
    //     CategoryID: 1,
    //     CreatedDate: new Date('2023-08-12'),
    //     selectedQuantity: 0,
    //   },
    //   {
    //     ID: 2,
    //     Name: 'SAMSUNG Note S23 Ultra',
    //     Quantity: 1,
    //     Price: 34999,
    //     Img: 'https://m.media-amazon.com/images/I/71Sa3dqTqzL._AC_UY327_FMwebp_QL65_.jpg',
    //     CategoryID: 1,
    //     CreatedDate: new Date('2021-08-05'),
    //     selectedQuantity: 0,
    //   },
    //   {
    //     ID: 3,
    //     Name: 'Apple iPhone 12 Pro Max',
    //     Quantity: 2,
    //     Price: 20000,
    //     Img: 'data:image/jpeg;base64,',
    //     CategoryID: 2,
    //     CreatedDate: new Date('2022-12-25'),
    //     selectedQuantity: 0,
    //   },
    //   {
    //     ID: 4,
    //     Name: 'Apple iPhone 13 Pro',
    //     Quantity: 3,
    //     Price: 43000,
    //     Img: 'https://m.media-amazon.com/images/I/610xvgzVUDL._AC_UL600_FMwebp_QL65_.jpg',
    //     CategoryID: 2,
    //     CreatedDate: new Date('2023-02-25'),
    //     selectedQuantity: 0,
    //   },
    //   {
    //     ID: 5,
    //     Name: 'Xiaomi Redmi Note 12',
    //     Quantity: 0,
    //     Price: 15000,
    //     Img: 'https://m.media-amazon.com/images/I/61X2GfAMHsL._AC_UY327_FMwebp_QL65_.jpg',
    //     CategoryID: 3,
    //     CreatedDate: new Date('2023-06-10'),
    //     selectedQuantity: 0,
    //   },
    //   {
    //     ID: 6,
    //     Name: 'Xiaomi Poco X5 PRO',
    //     Quantity: 10,
    //     Price: 9000,
    //     Img: 'https://m.media-amazon.com/images/I/51qXb8txLIL._AC_UY327_FMwebp_QL65_.jpg',
    //     CategoryID: 3,
    //     CreatedDate: new Date('2023-07-01'),
    //     selectedQuantity: 0,
    //   },
    // ];
    // this.categoryList = [
    //   { ID: 0, Name: 'All' },
    //   { ID: 1, Name: 'SAMSUNG' },
    //   { ID: 2, Name: 'Apple' },
    //   { ID: 3, Name: 'Xiaomi' },
    // ];
  }

  // @Output() categorySelected = new EventEmitter<ICategory>();
  // emitCategorySelected(categoryList: ICategory) {
  //   this.categorySelected.emit(categoryList);
  // }

  // useing search for price to filter
  @Input() set listFilterChild(value: string) {
    console.log(value);

    this.filteredList = this.prodService.performFilterVal(value);
  }
  // useing Category to filter

  @Input() set selectedCategoryIdChild(value: string) {
    console.log(value);
    if (value == '0') {
      this.filteredList = this.prodService.getAllProds();
    } else {
      this.filteredList = this.prodService.performFilterFun(value);
    }
    // console.log(this.filteredList);
  }
  // useing Price to filter

  @Input() set selectedCategoryIdPriceChild(value: string) {
    console.log(value);

    this.filteredList = this.prodService.performFilterFunPr(value);
  }
  // // Category to filter
  // filterFun(filterValue: string): IProduct[] {
  //   filterValue = filterValue.toLowerCase();
  //   return this.productsList.filter((product: IProduct) =>
  //     product.CategoryID.toString().includes(filterValue)
  //   );
  // }
  // // Price to filter
  // filterFunPr(filterValue: string): IProduct[] {
  //   filterValue = filterValue.toLowerCase();
  //   return this.productsList.filter((product: IProduct) =>
  //     product.Price.toString().includes(filterValue)
  //   );
  // }
  // // search for price to filter
  // filterVal(filterValue: string): IProduct[] {
  //   filterValue = filterValue.toLowerCase();
  //   return this.productsList.filter((product: IProduct) =>
  //     product.Price.toString().includes(filterValue)
  //   );
  // }
  @Output() newProdInCart = new EventEmitter<IProduct>();
  // buyProduct(product: IProduct, selectedQuantity: number) {
  //   if (product.Quantity > 0) {
  //     product.Quantity--;
  //     this.newProdInCart.emit(product);
  //   }
  // }

  buyProduct(product: IProduct, selectedQuantity: any) {
    // console.log(selectedQuantity);
    // product.selectedQuantity = selectedQuantity;
    // console.log(product);

    if (product.Quantity > 0 && selectedQuantity > 0) {
      if (selectedQuantity <= product.Quantity) {
        product.Quantity -= selectedQuantity;
        // Emit the product and selectedQuantity
        console.log('else 1111111');

        this.newProdInCart.emit(product);
      } else if (product.Quantity > 0 && selectedQuantity <= product.Quantity) {
        product.Quantity--;
        console.log('else 222222');

        this.newProdInCart.emit(product);
      } else  {
        throw 'Error loading';
      }
    } else if (product.Quantity > 0) {
      product.Quantity--;
      console.log('else 333333');

      this.newProdInCart.emit(product);
    }
  }

  toggleShow() {
    this.show = !this.show;
  }

  ngOnInit(): void {
    this.filteredList = Array.from(this.prodService.getAllProds());
  }
  date: Date = new Date();

  writeP() {
    this.credit = this.credit.slice(0, 16);
  }

  productDetails(id: number) {
    // console.log(id);
    this.router.navigate(['/single', id]);
  }
}
