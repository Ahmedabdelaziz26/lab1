...
export class ProductsService {
  ...
  getAllCatgs(): ICategory[] {
    return this.categoryList;
  }

  // Category to filter
  performFilterFun(filterValue: string): IProduct[] {
  ...
  // Price to filter
  performFilterFunPr(filterValue: string): IProduct[] {
    filterValue = filterValue.toLowerCase();
    return this.productsList.filter((product: IProduct) =>
  ...
  performFilterVal(filterValue: string): IProduct[] {
    filterValue = filterValue.toLowerCase();
    return this.productsList.filter((product: IProduct) =>
      product.Price.toString().includes(filterValue)
    );
  ...
  getPrdByID(productId: number): IProduct | undefined {
    return this.productsList.find((prd) => prd.id == productId);
  }
